...
### ⛔ טבלת מרחקי הקצה — נגישה, וקטלנית

`PsVolume.checkHoleEdgeDistance(Int32)` היא הבדיקה שהמדריך מתאר, ו-
`PsBaseObject.BlockHoleEdgeDistanceCheck` הוא דגל ההשתקה לחלק. **הקריאה הורגת את AutoCAD.**
בודדה בשלבים עם שמירה בכל שלב: פלטה נוצרה ושרדה, חור נקדח ושרד, **הקריאה לבדה על מודל שמור —
התהליך נעלם.** בלי חריגה ובלי דיאלוג.

⇒ **תשובה לשאלת הפרק: טבלת מרחקי הקצה נגישה בדיאלוג בלבד.**
⇒ ראה `knowledge/learning/findings/LETHAL-CALLS-do-not-invoke.md` — הקריאה השנייה במשפחה, אחרי
`PsPlate.computeObjectWeigth`.

### ✅ תחביר המחרוזת — `SetLinearHoleField` מקבל את תחביר הדיאלוג במלואו

ספירה לא מוכיחה כלום (`3*70` ו-`2*60,200,1*` נותנים שניהם 6 חורים). **המרווחים מוכיחים:**

```
x='3*70'          ->  מרווחים 70, 70        אחיד
x='2*60,200,1*'   ->  מרווחים 60, 200       לא אחיד, בדיוק כפי שנכתב
```

### ✅ `W` — מד הסימון עובד דרך ה-API

מוכח ב**ניגוד**, לא בספירה:

| קורה | `y=` | מרווח רוחבי שנמדד |
|---|---|---|
| **HE 300 B** | `2*W` | **120 מ"מ** |
| **IPE 300** | `2*W` | **80 מ"מ** |
| HE 300 B | `2*100` *(בקרה)* | **100 מ"מ** |

שני חתכים, אותה פקודה, **מד שונה** ⇒ `W` קורא את הטבלה של החתך עצמו. הבקרה הנומרית נוחתת על 100
בדיוק. **לא קפץ דיאלוג.**

⇒ ⭐⭐ **לרשת את המד, לא להמציא מרווחי ברגים.** זו החצי-קידוח של אותו עיקרון כמו "לבחור תבנית,
לא להעביר מספרים".
