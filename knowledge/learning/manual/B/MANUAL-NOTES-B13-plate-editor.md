...
## ‏AUDIT 10/08/2026 — שלושת הפריטים הפתוחים נסגרו

### ① ששת סוגי עיבוד הצלע — הנה הם

```
Bentley.ProStructures.EdgeLayout
   kUnknownEdge, kFacet(1), kRadius(2), kRounded(3), kInverted(4), kFold(5), kNotch(6)
```

כל השישה יושמו על שש פלטות (x = 410 000) ונקראו חזרה נכון — `layout=1…6`, `tv=25,25`, עליון
ותחתון שניהם. **מוכח, לא מצוטט.**

⚠️ ⭐⭐ **ממצא חדש — שני ה-APIs חלוקים.** התאום ב-COM חסר אחד:

```
.NET  EdgeLayout     ... kFold, kNotch     (7 כולל הסנטינל)
COM   KsEdgeLayout   ... kFold             (6)
```

**`kNotch` קיים ב-.NET בלבד.** זו מלכודת ה-enum של B.6 בצורה חדשה: שם — שני enums באותו שם
בהרכבות שונות; כאן — **אותו** enum עם **חברים שונים** בין שני ה-APIs.
⇒ **כששני ה-APIs חלוקים, ה-.NET הוא השלם.** ‏[[two-apis-com-wrapper]] צריך את הסייג ההפוך:
‏COM מציל את .NET כשהקישור נכשל, אבל הוא גם יכול להיות **מאחוריו**.

### ② `PsEdgeChamfer` — ניתן לבנייה, אין לו יוצר

כל המשטח שלו הוא דסטרוקטור ותכונות. אין `Create`, אין `insert`, אין `writeTo`. הוא **מטען
נתונים** שמוצמד ל-**`em.PlateBreakEdge`** על אובייקט המודיפיקציה. זה המסלול היחיד, וזה מה
שהתוסף כבר עושה.

### ③ `Min. Radius` / `Max. Height` — **לא נגישים**

היחידים במשטח ה-API כולו הם `MinimumRadius`/`EffectiveMinimumRadius` של **רשת רדיאלית ב-ISM**,
בלי קשר לצלעות פלטה. ⇒ **ולידציה בצד הדיאלוג בלבד**: התוכנה תגיד לאדם מה הגבולות ולא תגיד לקוד.

### חוט פתוח
`PsEditPlateModification.DisplayAsRaster` — שוב *Raster*, אותה מילה שמאחורי דגל הרשת של B.9.3.
ייתכן שזה מסלול הקריאה/כתיבה לדגל על פלטה קיימת, בעוד `SetGrid` מציע אותו רק ביצירה. **לא נבדק.**
