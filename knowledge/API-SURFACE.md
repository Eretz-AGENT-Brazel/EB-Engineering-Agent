...
161 DLL · 116 מנוהלים · 20,802 טיפוסים · 8,622 ציבוריים · 24,946 חתימות · 3 כשלונות
מפה גולמית מלאה : knowledge/API-SURFACE-RAW.txt      (3.0 MB)
אינדקס assemblies : knowledge/API-ASSEMBLY-INDEX.txt
```

## 📊 המספר — אחרי שבדקתי אותו פעמיים

**‏8,622 ציבוריים הוא מספר מטעה** — הוא כולל ספריות צד-שלישי שאינן ה-API של ProStructures.
הפירוט הכן:

| assembly | ציבוריים | רלוונטי? |
|---|---|---|
| `DocumentFormat.OpenXml` | 4,364 | ❌ ייצוא Office |
| `combit.ListLabel21.*` (5 קבצים) | ~2,150 | ⚠️ **מנוע הדוחות** — רשימות החלקים הן דוחות List&Label |
| **`Interop.ProSteel_COM`** | **631** | ✅ **API מקבילי ב-COM** (‏Ks_Com*), מעולם לא נגעתי |
| **`ProStructuresNet`** | **392** | ✅ הראשי |
| **`Bentley.Structural.Ism.Api.3.0`** | **184** | ✅ **ISM** — ממשק לתוכנות אנליזה |
| `PSN_*` · `PC3D*` · `PSX_PlugInBase` | ~200 | ✅ מאקרו החיבורים והבטון |
| `System.Data.SQLite.x64` | 69 | ✅ **פותר את בעיית ה-MDB** — יש נתיב SQLite |
| `aSa.PC.*` | ~160 | ⚠️ פרטי זיון (צד שלישי) |

| | |
|---|---|
| **המשטח הרלוונטי באמת** | **~1,400 טיפוסים ציבוריים** |
| טיפוסים שהסוכן משתמש בהם (v31) | **26** |
| **ניצולת** | **~1.9%** |

> **שלוש פעמים תיקנתי את המספר הזה:** 325 (assembly אחד) → 771 (75 assemblies) → 8,622 (הכול,
> אבל מזוהם) → **~1,400 (מסונן וכן).** בכל פעם הבדיקה הבאה הפריכה את הקודמת.
> **זה עצמו הלקח:** מספר שלא סוננתי אינו ממצא.
