...
| 🔒 **להצמיד את הסוכן למודל מסוים (ולוודא)** | [`WORKING-ON-MODELS.md`](WORKING-ON-MODELS.md) — שלוש הפקודות של אמיר: `assign` · `verify` · `unassign` |
...
├── WORKING-ON-MODELS.md    🔒 הנוהל של אמיר: להצמיד את הסוכן למודל אחד, ולוודא
...
│   ├── lessons/            שיעורים 1–7: תיעוד, לקחים, ביקורות
...
└── z-archive/              היסטוריה מתוארכת — לא נערכת לאחור
                            (ה-Z מכוונת: שומרת את הארכיון אחרון במיון של GitHub)
    ├── handoffs/           מסמכי מסירה בין סשנים — מלפני נוהל SESSION-START
    └── desktop-dot-claude-2026-08-17/
                            תוכן `Desktop\.claude` לפני מחיקתו בהוראת אמיר —
                            ובעיקרו **פורטל EB Mission Control**, שהיה עותק יחיד
```
...
> ### 0. **בר-ביצוע — הכלל שמעל כולם.** אמיר, 13/08/2026
> **"המודל הוא קודש הקודשים שלנו. הוא השפה המקצועית שלנו שאנחנו יודעים לדבר אחד עם השני.
> המקצועיות שלנו נבחנת כאן בדיוק — בדברים הקטנים… ואם משהו לא מובן שואלים, ואם אנחנו לא עומדים
...
> והוא חייב להיות בר-ביצוע.** חושבים כמו **הפועל במפעל** — ולו יש כל הכלים (חיתוך פרופילים,
> פלטות ב-CNC או בגיליוטינה, כיפוף, ניקוב), ולכן המגבלה אינה הכלי אלא **האם הפרט הגיוני
> ובר-הרכבה**. צריך לדעת **לעמוד מאחורי כל פרט ולהסביר למה הוא נוח לייצור**.
> ⛔ **אין "מידלתי טעות, לא שמתי לב"** — התוכניות נגזרות **אוטומטית מהמודל**, ולכן טעות מידול
> יוצאת כתוכנית חיתוך שגויה ומתגלה רק בסוף. שלוש השאלות לפני כל פרט ונוסח הכלל המלא:
> [`agent-brain/PROGRAM.md`](agent-brain/PROGRAM.md) § הכלל שמעל כולם.
