...
//   2. Python sends command  EB_RUN132
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds132))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp132))]
...
namespace EBAgent
{
    ...
    public class EBApp132 : IExtensionApplication
    {
    ...
    public class ApiCmds132
    {
        ...
        [CommandMethod("EB_RUN132", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
            ...
            try
            {
                    ...
                    switch (op)
                {
                    ...
                    case "vfy_grip": VfyGrip(kv); break;
        ...
        // op=vfy_grip [minx=] [maxx=] [tol=12] [slack=1.5]
        //
        // THE IRON RULE, ANSWERED BY PROSTEEL INSTEAD OF BY PROXIMITY.
        //
        // vfy_bolts matches a bolt to nearby holes and prints its own blind spot in every
        // result line: it cannot tell WHICH part a hole belongs to, so a hole in the wrong
        // part still counts. That blind spot is exactly where a real violation hides -- a
        // bolt clamping one drilled plate and one undrilled one looks fine to a proximity
        // matcher.
        //
        // E.9 supplied the missing number. PsObjectProperties.KlemmLen is the bolt's GRIP
        // LENGTH -- the manual (E.9.3) calls it "the calculated clamping length of the bolt",
        // ProSteel's own figure for the thickness of the packet the bolt clamps. And every
        // PsSingleHole carries Start and End, so a hole's DEPTH is the thickness of the
        // material it passes through. Therefore:
        //
        //     sum(depths of the holes matched to a bolt)   vs   KlemmLen
        //
        //     equal          -> every element the bolt clamps is drilled            OK
        //     sum < Klemm    -> the bolt clamps material that is NOT drilled     🧲 VIOLATION
        //     sum > Klemm    -> holes counted that the bolt does not pass through -- read the
        //                       coordinates; usually the tolerance swept in a neighbour
        //
        // Calibrated 10/08/2026 on the E.10 portal: M20x70 DIN6914, KlemmLen = 39, and the
        // packet is a 20 mm end plate plus a 19 mm HE300B flange. 20 + 19 = 39, exactly.
        //
        // ⚠️ Its own blind spot, stated because an instrument that never says "I don't know"
        // is the one that produced 32 false flags: it still matches holes to bolts by
        // PROXIMITY, so the SET of holes can be wrong even when the arithmetic is right. What
        // it adds is that the TOTAL THICKNESS must now agree with ProSteel's own figure -- a
        // wrong set of holes has to add up to exactly the right number to slip through.
        void VfyGrip(Dictionary<string, string> kv)
        {
            ...
            double slack = double.Parse(Get(kv, "slack", "1.5"), IC);
            ...
            // every hole in range: midpoint, DEPTH (|End-Start|) and owner
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("verdict\tbolt\tklemm\tdrilled\tdiff\tholes\towners");
            int ok = 0, undrilled = 0, over = 0, noKlemm = 0, noHole = 0;
            double worst = 0; string worstH = "";
            foreach (VPart bo in bolts)
            {
                double klemm = 0;
                try { PsObjectProperties pr = new PsObjectProperties(); pr.readFrom(bo.Id); klemm = pr.KlemmLen; }
                catch { }
                if (klemm <= 0)
                {
                    noKlemm++;
                    sb.AppendLine("NO-KLEMM\t" + bo.H + "\t0\t-\t-\t-\t(ProSteel reports no grip length)");
                    continue;
                }

                double drilled = 0; int hits = 0;
                ...
                for (int i = 0; i < hPt.Count; i++)
                {
                    ...
                    drilled += hDepth[i]; hits++;
                    if (own.Length > 0) own.Append(',');
                    own.Append(hOwner[i] + ":" + F(hDepth[i]));
                }

                double diff = klemm - drilled;
                string verdict;
                if (hits == 0) { verdict = "BOLT-NO-HOLE"; noHole++; }
                else if (diff > slack) { verdict = "UNDRILLED"; undrilled++; }
                else if (diff < -slack) { verdict = "OVER-COUNT"; over++; }
                else { verdict = "OK"; ok++; }
                if (Math.Abs(diff) > Math.Abs(worst)) { worst = diff; worstH = bo.H; }

                sb.AppendLine(verdict + "\t" + bo.H + "\t" + F(klemm) + "\t" + F(drilled)
                            + "\t" + F(diff) + "\t" + hits + "\t" + own.ToString());
            }

            File.WriteAllText(Path.Combine(Path.GetDirectoryName(
                Assembly.GetExecutingAssembly().Location), "eb_vfy_grip.txt"),
                sb.ToString(), new UTF8Encoding(true));

            Result("EB_" + ((undrilled > 0 || noHole > 0) ? "ERR" : "OK")
                 + " vfy_grip bolts=" + bolts.Count + " OK=" + ok
                 + "  UNDRILLED=" + undrilled + " (🧲 the bolt clamps material with NO hole)"
                 + "  BOLT-NO-HOLE=" + noHole
                 + "  OVER-COUNT=" + over + " (holes counted that it does not cross)"
                 + "  no-klemm=" + noKlemm
                 + "  worst=" + F(worst) + "mm (" + worstH + ")  slack=" + F(slack)
                 + "  [grip length is ProSteel's OWN figure for the clamped packet; the hole SET"
                 + " is still matched by proximity, so a wrong set must add up to exactly the"
                 + " right total to slip through] -> eb_vfy_grip.txt");
        }
        ...
        void VfyBolts(Dictionary<string, string> kv)
        {
            ...
            Result(((boltNoHole == 0) ? "EB_OK" : "EB_ERR") + " vfy_bolts bolts=" + bolts.Count +
                   ...
                   "  BOLT-NO-HOLE=" + boltNoHole + " (🧲 iron rule)" +
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId oid in ms)
                {
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        ...
                        // ⚠️ MEASURED 09/08: LinkType cannot be read this way. PsEditConnection
        ...
        void BoltInfo(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                // ⚠️ The dump prints BoltStyle / BoltType / Diameter as plain properties.
        ...
        // =====================================================================
        //  v108 — B.24 DYNAMIC BRACING  (PS_VERBAND)
        ...
        //
        //  ⚠️ The names are cross-wired. BracingLayout is the manual's "Shape
        ...
        void Bracing(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                // ⚠️ takes the enum NAMED ShapePosition, but it means the PLATE's position
                v = Get(kv, "plateside", "");
                ...
                // ⭐ B.24: "first place your USER COORDINATE SYSTEM OVER THE BRACING PLANE
        ...
        void Weld(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                // ⭐ the weld itself, not just the flag
        ...
        // =====================================================================
        //  v106 — B.21 SPLICE JOINTS  (PS_LASCHE)
        ...
        //  ⚠️ Hard precondition the siblings lack: "Both of the shapes have to be
        //  IN ALIGNMENT along the surfaces to be connected." Collinear members only.
        //  ⚠️ Spelling: HoleWorkloose here, HoleWorkLoose on the other two classes.
        // =====================================================================
        ...
        void Splice(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                // ⚠️ HoleWorkloose -- lowercase l, unlike the sibling classes
            ...
            foreach (string h in HandleSet())
                if (!pre.Contains(h)) { made.Append(" " + h); n++; }
            Result(((after > before) ? "EB_OK" : "EB_ERR") + " splice a=" + Get(kv, "handle", "") +
                   " census=" + before + "->" + after + " newObjects=" + n +
                   " GetPlateId:[" + plates + " ]" +
                   (n <= 30 ? " handles:" + made : "") + applied + msg);
        }

        // =====================================================================
        //  v105 — B.20 SHEAR PLATES  (PS_SCHEARPLATE)
            ...
            try
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    try
                    {
                        ...
                        // ⭐ the lead B.19 lacked: ask whether a cope-template name is valid.
        ...
        void ShearPlate(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                if (v.Length > 0)
                {
                    // ⭐ validate BEFORE applying -- B.19's cope failed silently with no way
            ...
            foreach (string h in HandleSet())
                if (!pre.Contains(h)) { made.Append(" " + h); n++; }
            Result(((after > before) ? "EB_OK" : "EB_ERR") + " shearplate beam=" + Get(kv, "handle", "") +
                   " census=" + before + "->" + after + " newObjects=" + n +
                   " GetPlateId:[" + plates + " ]" +
                   (n <= 24 ? " handles:" + made : "") + applied + copeChk + msg);
        }

        // =====================================================================
        //  v103 — B.19 WEB ANGLE  (PS_STEGW)
        ...
        void WebAngle(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                v = Get(kv, "dia", "");         if (v.Length > 0) { d.HoleDiameter = double.Parse(v, IC); applied += " dia=" + v; }
                v = Get(kv, "workloose", "");   if (v.Length > 0) { d.HoleWorkLoose = double.Parse(v, IC); applied += " workloose=" + v; }
                // ⭐ BoltStyle is a STRING here -- the stiffener class exposed only a CRC
                v = Get(kv, "boltstyle", "");   if (v.Length > 0) { d.BoltStyle = v; applied += " boltStyle=" + v; }
                ...
                v = Get(kv, "weldconn", "");    if (v.Length > 0) { d.WeldToConnectedShape = (v == "1"); applied += " weldConn=" + v; }
                v = Get(kv, "weldsup", "");     if (v.Length > 0) { d.WeldToSupportShape = (v == "1"); applied += " weldSup=" + v; }
                // ⭐ the DAST load fields -- the first load-carrying properties in this API
                v = Get(kv, "shear", "");       if (v.Length > 0) { d.ShearZ = double.Parse(v, IC); applied += " shearZ=" + v; }
                ...
                // ⭐ this class can report what it made -- most connection classes cannot
                try
                {
                    for (int i = 0; i < 8; i++)
                    {
        ...
        // =====================================================================
        //  v100 — B.16 INSERT STIFFENERS  (PS_RIP)
        ...
        // ===================================================================
        //  E.9  ProSteel Properties Dialogs — the dialog, turned into code
        // ===================================================================
        ...
        // PsObjectProperties, and that class also carries writeTo(Int64 ObjId) —
        ...
        //     properties valid for these parts" — hence the tabs below: a bolt has
        ...
        // named in E.9 falls to 8-Other rather than being dropped — the point of the map
        // is that nothing is lost, not that everything is classified.
        static string E9Tab(string p)
        {
            switch (p)
            {
                // E.9.1/2/3 "Layout" — how the part is DRAWN, nothing structural
                ...
                case "HasUserMidpointDefined":
                    return "3-Position";
                // E.9.1/2/3 "Data" — the parts-list identity
                ...
                case "Partart": case "FreeDescription": case "Handle":
                    return "4-Data";
                // E.9.1/2/3 "Values" — the numbers. KlemmLen is the bolt's GRIP LENGTH
                ...
                // Not on any tab — reactor and detailing state the dialog hides
        ...
        void PropFull(Dictionary<string, string> kv)
        {
            ...
            sb.AppendLine("(rw = writable, and writeTo(oid) commits it — see op=propset)");
        ...
        // "Match properties" for ProSteel, built on copyFrom(). DEFAULT IS TABS 4+6 —
        ...
        // PsObjectProperties.ChangeShapeType(oid, Key, Katalog, ShapeType) — swap the
        ...
        // and without losing whatever hangs off it. The section key is an OPAQUE STRING —
        ...
        void ChangeSection(Dictionary<string, string> kv)
        {
            ...
            // resolve the catalogue the way `beam` does — search, never assemble
            if (cat.Length == 0)
            {
                try
                {
            ...
            // The enum members are kNormalType/kWeldType/..., NOT eShape* — measured from
        ...
        //   radial    : radius=          (SetAsRadialPlate — a circular plate)
        //   diagonal  : p1= p2=          (SetFromRectangle — "inserted according to its diagonal")
        //   fromshape : handle=          (CreateFromShape — "a flat is transformed into a
        ...
        void ArcPlate(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                string lay = Get(kv, "layer", "");
                if (lay.Length > 0) { ap.SetLayer(lay); ap.UseCurrentLayer(false); } else ap.UseCurrentLayer(true);
                string nm = Get(kv, "name", "");
        ...
        //
        // ⚠️ MEASURED 08/08: PsCreateBendPlate.Create() returns FALSE and yet the flat
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (pl != null && rad <= pl.Height / 2.0)
                                applied += " ⚠radius<=t/2(" + F(pl.Height / 2.0) + ")";
                        }
                        catch { }
        ...
        void BendInfo(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    // ⚠️ MEASURED 08/08: AddFlange returned flangeIndex=2 and the model
                    ...
                    for (int i = 0; i < scan; i++)
                    {
                        try
                        {
                            ...
                            // ⚠️ AddFlange TAKES DEGREES, Flange.Angle RETURNS RADIANS:
                    ...
                    // ⭐ THE DEPENDENCY TREE. The dump prints "P Int32 ParentFlangeIndex"
        ...
        void Frame(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                //   Type 0/1 = 123 vs ABC · Display = text/circle/block · Order = decreasing
                //   Position = front/rear/left/right · Start = first value
                //   DoubleLine = "2 Lines" · Dynamic = names follow the view direction
        ...
        // with previously defined joints… insert the column at the intersection point of the
        // WORK FRAME AXES (grid) which are situated within a rectangular area… [girders are]
        // inserted along the axes… finally, these are divided at all intersection points."
        //
        // ⇒ Twenty columns on a grid is ONE operation against the work frame, not a
        ...
        //
        // ⭐ And the rule from B.8.1 that explains the orientation surprises:
        ...
        // Amir, 06/08/2026: "אני מייצר חורים בהתאם למה שאני צריך בפקודת DRILL, ולאחר מכן
        ...
        //
        // ⚠️ THE MISTAKE THIS FIXES: PsCreateBolt has TWO paths, and this agent had only ever
        ...
        {
            System.Globalization.CultureInfo IC = System.Globalization.CultureInfo.InvariantCulture;
            // ⚠️ QUARANTINED 06/08/2026 -- THIS OP CRASHES AutoCAD.
        ...
        // ⚠️ HARD ORDERING RULE from the manual, B.29.1 "Group Detection":
        ...
        // ⚠️ The manual's warning that makes this necessary at all (B.12.7, p.225): ProSteel
        ...
        static readonly Dictionary<string, string> OpKeys = new Dictionary<string, string>()
        {
            ...
            { "vfy_grip", "|maxx|minx|tol|slack|" },
            { "vfy_touch", "|a|b|tol|" },
            { "vfy_size", "|max|maxx|minx|" },
            { "classify", "|maxx|minx|out|set|value|visible|" },
            { "acis", "|handle|" },
            { "acisref", "|massprop|read|solid|" },
            { "solid", "|at|axis1|axis2|center|ex|ey|h|inner|kind|l|len|normal|oi1|oi2|oo1|oo2|outer|p1|p2|pts|r|r1|r2|rev|taper|twist|w|" },
            ...
            { "bracing", "|angle|cat|centerhole|cross|divideall|dm|dynamic|edgeborder|edgehole|ex|ey|group|holecross|holeedge|holehole|host1|host2|layout|mirror|ncross|nogussets|nprof|origin|p1|p2|plateside|platethick|platetype|platewide|play|roundto|shapedist|shapetype|shorten|size|sym|type|prerecalc|welded|setucs|" },
            { "weldstyles", "||" },
