...
//   2. Python sends command  EB_RUN55
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds55))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp55))]
...
namespace EBAgent
{
    ...
    public class EBApp55 : IExtensionApplication
    {
    ...
    public class ApiCmds55
    {
        ...
        [CommandMethod("EB_RUN55", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        void CloneDrills(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                // TakeoverDrills is Void and the selections are provably correct, so the call
                // sequence is what is left. PsDrillObject's shape is: many Set*, ONE executor
                // Apply(). variant= lets one rebuild test four sequences instead of four
                // rebuilds testing one -- the DLL locks on NETLOAD, so a rebuild is expensive.
                int variant = int.Parse(Get(kv, "variant", "1"));
                PsDrillObject d = new PsDrillObject();
                int applyRc = -999;
                switch (variant)
                {
                    ...
                    case 9:  // MY OWN implementation -- read the source holes, make them again
                        {    // on each target. Both halves are already proven: PsSingleHoleArray
                             // reads, and SetSingleHoleField + Apply creates.
                             // Translation only. The manual warns the real Clone works in the
                             // PART coordinate system, so a rotated or mirrored target would
                             // receive mirrored holes -- here such a target is REFUSED, not
                             // silently drilled wrong.
                            PsPoint sMin = new PsPoint(0, 0, 0), sMax = new PsPoint(0, 0, 0);
                            Bentley.ProStructures.Property.PsObjectProperties ps =
                                new Bentley.ProStructures.Property.PsObjectProperties();
                            ps.readFrom(src); ps.GetExtents(ref sMin, ref sMax);
                            ...
                            {
                                PsPoint tMin = new PsPoint(0, 0, 0), tMax = new PsPoint(0, 0, 0);
                                Bentley.ProStructures.Property.PsObjectProperties pt =
                                    new Bentley.ProStructures.Property.PsObjectProperties();
                                pt.readFrom(tgt[t]); pt.GetExtents(ref tMin, ref tMax);
                                double dxS = System.Math.Abs((sMax.x - sMin.x) - (tMax.x - tMin.x));
                                double dyS = System.Math.Abs((sMax.y - sMin.y) - (tMax.y - tMin.y));
                                double dzS = System.Math.Abs((sMax.z - sMin.z) - (tMax.z - tMin.z));
                                if (dxS > 0.5 || dyS > 0.5 || dzS > 0.5)
                                { skipped++; ex += " " + tgtH[t] + ":ORIENTATION-DIFFERS-refused"; continue; }
                                double ox = tMin.x - sMin.x, oy = tMin.y - sMin.y, oz = tMin.z - sMin.z;
                                for (int i = 0; i < nh; i++)
                                {
                                    ...
                                    dd.SetInsertPoint(new PsPoint(a0.x + ox, a0.y + oy, a0.z + oz));
                                    dd.SetNormal(new PsVector(b0.x - a0.x, b0.y - a0.y, b0.z - a0.z));
                                    // dm is the measured HOLE diameter; workloose 0 so the new
                                    // hole is that size and not that size plus a bolt clearance.
        ...
        void Styles(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                lst.Initialize();
                try { lst.Synchronize(true); } catch { }
                try { lst.ReadFromFile(); } catch { }
                n = lst.Count;
