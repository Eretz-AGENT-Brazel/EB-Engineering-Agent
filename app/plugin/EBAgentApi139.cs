...
//   2. Python sends command  EB_RUN139
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds139))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp139))]
...
namespace EBAgent
{
    ...
    public class EBApp139 : IExtensionApplication
    {
    ...
    public class ApiCmds139
    {
        ...
        [CommandMethod("EB_RUN139", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        void Env(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                foreach (Assembly a in asms)
                {
                    ...
                    // ⚠️ AN ASSEMBLY HAS TWO VERSIONS AND THEY ARE NOT THE SAME NUMBER.
        ...
        // you don't have to take care of this… objects are created on their own layer." On
        // 09/08 I measured 88 parts sitting on layer 0 — every one made by calling a Ps*Create*
        // class directly — and FIXED THE MODEL by moving them. The model has stayed clean
        ...
        void PropFull(Dictionary<string, string> kv)
        {
            ...
            sb.AppendLine("(rw = the .NET property HAS A SETTER. It does NOT mean the value"
                        ...
                        + " ⇒ only op=propset's READ-BACK tells you which.)");
