...
//   2. Python sends command  EB_RUN89
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds89))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp89))]
...
namespace EBAgent
{
    ...
    public class EBApp89 : IExtensionApplication
    {
    ...
    public class ApiCmds89
    {
        ...
        [CommandMethod("EB_RUN89", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        static List<PsPoint> GridPoints(long gid, out string note)
        {
            ...
            try
            {
                ...
                // a polygon covering the whole frame, generously expanded
                PsPolygon poly = new PsPolygon();
                poly.init();
                double pad = 10.0;
                poly.appendVertex(mn.x - pad, mn.y - pad, 0);
                poly.appendVertex(mx.x + pad, mn.y - pad, 0);
                poly.appendVertex(mx.x + pad, mx.y + pad, 0);
                poly.appendVertex(mn.x - pad, mx.y + pad, 0);
                poly.Close();
                PsDataPointArray arr = new PsDataPointArray();
                int n = g.getPointsInsidePoly(poly, arr);
                note = "getPointsInsidePoly=" + n;
                for (int i = 0; i < n; i++)
                {
        ...
        void GridColumns(Dictionary<string, string> kv)
        {
            ...
            string note;
            List<PsPoint> pts = GridPoints(gid, out note);
            if (pts.Count == 0) { Result("EB_ERR gridcolumns: no grid intersections (" + note + ")"); return; }
        ...
        static readonly Dictionary<string, string> OpKeys = new Dictionary<string, string>()
        {
            ...
            { "gridpoints", "|handle|" },
            { "gridcolumns", "|catalog|h|handle|name|xpos|ypos|" },
