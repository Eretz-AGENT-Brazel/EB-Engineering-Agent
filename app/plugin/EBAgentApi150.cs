...
//   2. Python sends command  EB_RUN150
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds150))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp150))]
...
namespace EBAgent
{
    ...
    public class EBApp150 : IExtensionApplication
    {
    ...
    public class ApiCmds150
    {
        ...
        [CommandMethod("EB_RUN150", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        void EdgeCheck(Dictionary<string, string> kv)
        {
            ...
            if (oid == 0) { Result("EB_ERR edgecheck: bad handle"); return; }
            Document doc = Application.DocumentManager.MdiActiveDocument;
            StringBuilder sb = new StringBuilder();
            int nf = -1, bad = 0, ok = 0;
            string blockState = "n/a";
            try
            {
            ...
            catch (System.Exception e) { sb.Append(" fieldCount!").Append(One(e.Message)); }
            ...
            using (Transaction tr = doc.Database.TransactionManager.StartTransaction())
            {
                try
                {
                    ObjectId id = doc.Database.GetObjectId(false,
                        new Handle(Convert.ToInt64(Get(kv, "handle", ""), 16)), 0);
                    DBObject o = tr.GetObject(id, OpenMode.ForRead);
                    Bentley.ProStructures.Entity.PsVolume vol = o as Bentley.ProStructures.Entity.PsVolume;
                    if (vol == null) sb.Append(" NOT a PsVolume (").Append(o == null ? "null" : o.GetType().Name).Append(")");
                    else
                    {
                        Bentley.ProStructures.Entity.PsBaseObject bo = o as Bentley.ProStructures.Entity.PsBaseObject;
                        if (bo != null)
                        {
                            try { blockState = bo.BlockHoleEdgeDistanceCheck.ToString(); } catch { }
                            string wb = Get(kv, "block", "");
                            if (wb.Length > 0)
                            {
                                try
                                {
                                    DBObject ow = tr.GetObject(id, OpenMode.ForWrite);
                                    Bentley.ProStructures.Entity.PsBaseObject bw = ow as Bentley.ProStructures.Entity.PsBaseObject;
                                    bw.BlockHoleEdgeDistanceCheck = (wb == "1");
                                    blockState = bw.BlockHoleEdgeDistanceCheck.ToString() + "(set)";
                                }
                                catch (System.Exception e) { sb.Append(" block!").Append(One(e.Message)); }
                            }
                        }
                        int upto = nf > 0 ? nf : 1;
                        ...
                        {
                            int rc = -999;
                            try { rc = vol.checkHoleEdgeDistance(i); }
                            catch (System.Exception e) { sb.Append(" f").Append(i).Append("!").Append(One(e.Message)); continue; }
                            sb.Append(" f").Append(i).Append("=").Append(rc);
                            if (rc == 0) ok++; else bad++;
                        }
                    }
                }
                catch (System.Exception ex) { sb.Append(" EX:").Append(One(ex.Message)); }
                tr.Commit();
            }
            Result("EB_OK edgecheck handle=" + Get(kv, "handle", "") + " holeFields=" + nf +
                   " rc0=" + ok + " rcOther=" + bad + " block=" + blockState + sb.ToString());
        }
