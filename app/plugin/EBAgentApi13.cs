...
//   2. Python sends command  EB_RUN13
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds13))]
...
namespace EBAgent
{
    ...
    public class ApiCmds13
    {
        ...
        [CommandMethod("EB_RUN13", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
            ...
            for (int strat = 0; strat < 2; strat++)  // MirrorFlag is read-only (compiler-confirmed)
            {
                try
                {
                    ...
                    using (Transaction t = dm.Database.TransactionManager.StartTransaction())
                    {
                        ...
                        if (strat == 0) ns.YMirrorFlag = true;
                        else ns.SetShapeMirror();
                ...
                try
                {
                    ...
                    using (Transaction t2 = dm2.Database.TransactionManager.StartTransaction())
                    {
                        ...
                        if (!got && rs != null) { try { got = rs.YMirrorFlag; } catch { } }
                        t2.Commit();
