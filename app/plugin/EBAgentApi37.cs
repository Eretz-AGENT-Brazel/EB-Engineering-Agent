...
//   2. Python sends command  EB_RUN37
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds37))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp37))]
...
namespace EBAgent
{
    ...
    public class EBApp37 : IExtensionApplication
    {
    ...
    public class ApiCmds37
    {
        ...
        [CommandMethod("EB_RUN37", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        void Conn(Dictionary<string, string> kv)
        {
            ...
            // Status is derived from the MEASURED census delta, never from what Create returned.
            string tail = " kind=" + kind + " beamId=" + bId + " supId=" + sId +
                          " rc=" + rc + " create=" + made +
                          " delta=" + delta + " plates=" + plates +
                          " before=" + before + " after=" + after +
                          (plateIds.Length > 0 ? " plateIds=" + plateIds : "") + msg;
            if (delta > 0) Result("EB_OK conn" + tail);
            else Result("EB_ERR conn produced nothing." + tail);
        }
