...
//   2. Python sends command  EB_RUN12
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds12))]
...
namespace EBAgent
{
    ...
    public class ApiCmds12
    {
        ...
        [CommandMethod("EB_RUN12", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
                        ...
                        if (after > before)
                        {
                            if (mirror)
                            {
                                try
                                {
                                    ...
                                    {
                                        long hh = Convert.ToInt64(h1, 16);
                                        ObjectId oid = dm.Database.GetObjectId(false, new Handle(hh), 0);
                                        PsShape ns = t2.GetObject(oid, OpenMode.ForWrite) as PsShape;
                                        if (ns != null) ns.SetShapeMirror();
                                        t2.Commit();
                                    }
                                }
                                catch { }
