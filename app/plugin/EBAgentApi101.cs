...
//   2. Python sends command  EB_RUN101
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds101))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp101))]
...
namespace EBAgent
{
    ...
    public class EBApp101 : IExtensionApplication
    {
    ...
    public class ApiCmds101
    {
        ...
        [CommandMethod("EB_RUN101", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
                        ...
                        try
                        {
                            if (pl != null)
                            {
                                ...
                                else if (w == "bulge")
                                {
                                    ...
                                    {
                                        double vx = 0, vy = 0, bg = 0;
                                        try
                                        {
                                            if (poly.getVertexbyValue(q, ref vx, ref vy, ref bg))
                                                sb.Append((q > 0 ? " " : "") + F(vx) + "," + F(vy) +
                                                          (System.Math.Abs(bg) > 1e-9 ? ("/bulge=" + F(bg)) : ""));
                                        }
                                        catch (System.Exception e) { sb.Append(" !EX:" + One(e.Message)); break; }
                                    }
                                    sb.Append("]");
                                }
