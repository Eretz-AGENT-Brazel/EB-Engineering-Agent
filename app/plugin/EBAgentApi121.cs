...
//   2. Python sends command  EB_RUN121
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds121))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp121))]
...
namespace EBAgent
{
    ...
    public class EBApp121 : IExtensionApplication
    {
    ...
    public class ApiCmds121
    {
        ...
        [CommandMethod("EB_RUN121", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        void ConnVerify(Dictionary<string, string> kv)
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId oid in ms)
                {
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        ...
                        try
                        {
                            PsEditConnection ec = new PsEditConnection();
                            type = ec.LinkType.ToString();
                        }
                        catch { }

                        // the cross-check B.27's yellow light is really about
                        string verdict = "ok";
                        if (nholes > 0 && nbolt == 0) { verdict = "HOLES-NO-BOLTS"; flagged++; }
                        else if (nbolt > 0 && nholes == 0) { verdict = "BOLTS-NO-HOLES"; flagged++; }
            ...
            Result("EB_OK connverify parts=" + parts + " withLinks=" + withLinks +
                   " links=" + links + " flagged=" + flagged + " -> " + outn);
        }
