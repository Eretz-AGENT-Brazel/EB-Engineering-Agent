...
//   2. Python sends command  EB_RUN95
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds95))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp95))]
...
namespace EBAgent
{
    ...
    public class EBApp95 : IExtensionApplication
    {
    ...
    public class ApiCmds95
    {
        ...
        [CommandMethod("EB_RUN95", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        // op=bend handle=<flat plate> at=x,y,z len= [front=0 rear=0] radius= angle=
        //         [lengthcalc=Absolut|Outer|Inner|Center] [inner=1]
        // B.9.4: "To generate a bent plate, you FIRST HAVE TO HAVE INSERTED A FLAT PLATE.
        // This plate determines the alignment." PsCreateBendPlate converts it; AddFlange
        // then carries exactly the six dialog fields. `at` is the click that picks the
        // REFERENCE EDGE -- "The new plate segment is ALWAYS SUBORDINATE to the reference
        // segment."
        void Bend(Dictionary<string, string> kv)
        {
            ...
            try
            {
                // step 1 -- turn the flat plate into a bent plate
                PsCreateBendPlate cb = new PsCreateBendPlate();
                cb.SetToDefaults();
                cb.SetObjectId(pid);
                bool made = cb.Create();
                applied += " convert=" + made;
                long bid = 0;
                try { bid = cb.ObjectId; } catch { }
                if (bid == 0) bid = pid;
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    if (bp == null) { msg = " NOT a PsBendPlate after convert (" +
                    ...
                    else
                    {
                        ...
                        // B.9.4 warns the radius must exceed half the plate thickness
                        try
                        {
                            PsPlate pl = o as PsPlate;
            ...
            Result(((cnt > 0) ? "EB_OK" : "EB_ERR") + " bend handle=" + Get(kv, "handle", "") +
                   " AddFlange rc=" + rc + " flangeIndex=" + flangeIdx + " flangeCount=" + cnt +
                   applied + msg);
        }
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    for (int i = 0; i < n && i < 12; i++)
                    {
                        try
                        {
                            ...
                            sb.Append(" | [" + i + "] len=" + F(f.Length) + " ang=" + F(f.Angle) +
        ...
        // op=plateinfo handle=  -- everything the chapter says lands on the parts list
        void PlateInfo(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    if (pl != null)
                    {
                        sb.Append(" thickness=" + F(pl.Height) +
                                  " insertHeight=" + F(pl.InsertHeight) +
                                  " insertXY=" + F(pl.InsertX) + "," + F(pl.InsertY));
                        try { sb.Append(" weight=" + F(pl.computeObjectWeigth(true))); } catch { }
                        // B.9.1 "Analysis of Dimensions": parallel edges -> right-angled
                        // corner -> longest side. This is the number that reaches the list.
                        try { sb.Append(" dimension=" + F(pl.computeObjectDimension(true))); } catch { }
                        try { sb.Append(" paintArea=" + F(pl.computeObjectPaintArea())); } catch { }
                        try { sb.Append(" dimAlign=" + pl.DimensionAlignement); } catch { }
                        try { PsVector dv = pl.DimensionDirection;
                              sb.Append(" dimDir=" + F(dv.x) + "," + F(dv.y) + "," + F(dv.z)); } catch { }
                        try { sb.Append(" name='" + pl.Name + "'"); } catch { }
                        try { sb.Append(" nameTpl='" + pl.getConvertedName(PlateNameTemplateType.kNameTemplate) + "'"); } catch { }
                        try
                        {
                            PsPolygon poly = new PsPolygon(); poly.init();
                            pl.GetPolygon(poly);
                            sb.Append(" polyVerts=" + poly.Count);
                            try { sb.Append(" crossCheck=" + poly.crossCheck(false)); } catch { }
                            try { sb.Append(" isClosedGapFree=" + poly.checkDoubleVertexes(0.01, false)); } catch { }
                        }
                        catch (System.Exception e) { sb.Append(" poly EX:" + One(e.Message)); }
                    }
                    PsArcPlate arc = o as PsArcPlate;
                    if (arc != null)
                        sb.Append(" | ARC radius=" + F(arc.Radius) + " neutralRadius=" + F(arc.NeutralRadius) +
                                  " width=" + F(arc.Width) + " height=" + F(arc.Height) +
                                  " startAng=" + F(arc.StartAngle) + " endAng=" + F(arc.EndAngle) +
                                  " turnAng=" + F(arc.TurnAngle));
                    PsBendPlate bp = o as PsBendPlate;
                    if (bp != null) { try { sb.Append(" | BEND flangeCount=" + bp.FlangeCount); } catch { } }
                    // WCS extents -- the only honest geometric read-back
                    try
                    {
                        PsObjectProperties p = new PsObjectProperties();
                        p.readFrom(pid);
        ...
        void Purlin(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                long p2  = IdFromHandle(Get(kv, "p2", ""));
                if (sup == 0 || p1 == 0 || p2 == 0)
                { Result("EB_ERR purlin: need support= (the girder) and p1= p2= (the two purlin runs)"); return; }
                ...
                string h1, c1; int after = Census(out h1, out c1);
                int delta = after - before;
                Result((delta > 0 ? "EB_OK" : "EB_ERR") + " purlin template='" + tmpl + "'" +
        ...
        static readonly Dictionary<string, string> OpKeys = new Dictionary<string, string>()
        {
            ...
            { "bend", "|angle|at|front|handle|inner|len|lengthcalc|radius|rear|" },
            { "bendinfo", "|handle|" },
