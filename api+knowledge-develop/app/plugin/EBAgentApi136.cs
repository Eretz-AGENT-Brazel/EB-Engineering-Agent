...
//   2. Python sends command  EB_RUN136
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds136))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp136))]
...
namespace EBAgent
{
    ...
    public class EBApp136 : IExtensionApplication
    {
    ...
    public class ApiCmds136
    {
        ...
        [CommandMethod("EB_RUN136", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        //
        // ⭐⭐ WHY THIS MATTERS FOR EVERY OTHER CHAPTER. A.1.2's own worked example reads:
        //     "ProSteel V8i (SELECTseries 3) - Version 8.11.3.48 dated from Aug 25 2010"
        // and this installation is Ss6 R1, ProStructuresNet 08.11.11.161, © 2013. Same 8.11,
        // but build 3.48 against 11.161 -- eight SelectSeries apart and three years later.
        // ⇒ **The manual documents an OLDER program than the one we run.** That is not a
        ...
        // ⇒ **"The manual says X" always carries an unstated "…in SS3".**
        void Env(Dictionary<string, string> kv)
        {
            bool full = Get(kv, "full", "0") == "1";
            StringBuilder sb = new StringBuilder();
            string psVer = "?", psFile = "?", acVer = "?";
            ...
            try
            {
                ...
                foreach (Assembly a in asms)
                {
                    ...
                    if (nm == "ProStructuresNet") { psVer = ver; psFile = loc; }
                    rows.Add(string.Format("  {0,-34} {1,-18} {2}", nm, ver, loc));
                }
                rows.Sort();
            ...
            sb.AppendLine("  what is running here             :  ProStructuresNet " + psVer);
            ...
            Result("EB_OK env ProStructuresNet=" + psVer + " AutoCAD=" + acVer
        ...
        //
        // ⭐ A field is not a hole. On AE6 the audit found 12 fields carrying 12 holes -- one
        ...
        //
        // ⚠️ B.26's notes said holes could not be removed. That was wrong -- DeleteHoleField
        ...
        //
        // ⚠️ THIS OP REPLACES THE FIRST vfy_grip, WHICH WAS WRONG. Recorded because the
        ...
        // ⇒ KlemmLen is essentially a property of the bolt TYPE and LENGTH. It coincides with
        ...
        void VfyFit(Dictionary<string, string> kv)
        {
            ...
            foreach (VPart bo in bolts)
            {
                ...
                // ⭐ THE GAP. A hole spans exactly the material it passes through, so consecutive
            ...
            Result("EB_" + ((noHole > 0 || over > 0) ? "ERR" : "OK")
                 ...
                 + "  BOLT-NO-HOLE=" + noHole + " (🧲 iron rule)"
                 + "  GAP-IN-PACKET=" + gapped + " (⚠ the plies do NOT touch: worst "
