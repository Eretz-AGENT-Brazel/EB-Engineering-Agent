...
//   2. Python sends command  EB_RUN87
...
[assembly: CommandClass(typeof(EBAgent.ApiCmds87))]
[assembly: ExtensionApplication(typeof(EBAgent.EBApp87))]
...
namespace EBAgent
{
    ...
    public class EBApp87 : IExtensionApplication
    {
    ...
    public class ApiCmds87
    {
        ...
        [CommandMethod("EB_RUN87", CommandFlags.Modal)]
        public void Run()
        {
            var kv = ReadCmd();
            string op = Get(kv, "op", "");
        ...
        void Shape(Dictionary<string, string> kv)
        {
            ...
            try
            {
                ...
                if (flat.Length > 0)
                {
                    // B.8.1 "Key ... to be able to create NON-STANDARDISED shape sizes of
                    // tubes, flat steel, round iron". CreateFlatSteel is that route for flats.
                    string[] wt = flat.ToLowerInvariant().Split('x');
                    cs.CreateFlatSteel(double.Parse(wt[0], IC), double.Parse(wt[1], IC));
                    applied += " flat=" + flat;
                }
                else
                {
