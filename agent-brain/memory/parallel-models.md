...
metadata: 
  ...
  modified: 2026-08-17T11:40:01.272Z
---
...
3. 🔒 **כשאמיר מקצה (בצ'אט: "אתה עובד רק על X — נעל את עצמך אליו", או `assign`) — אני מריץ
   את `assign` + `verify` בעצמי, מדווח, ולא יוצא מהסט בשום מסלול.** ‏`assign` מקבל גם
   רשימה עם משימה לכל מודל (סיטואציה 2 שלו). רק `unassign` שלו משחרר. הנוהל שלו:
   `WORKING-ON-MODELS.md` בשורש הריפו.
4. **סדר ההפעלה כשעובדים במקביל: ה-AutoCAD שלי ראשון, שלו שני.**
5. ⛔⛔ **רק מופע AutoCAD אחד נגיש ל-COM בכל רגע** — המוקדם שעדיין חי. שתי רשומות ב-ROT
...
6. **‏`reachable=0` בזמן ש-acad.exe חי = דיאלוג חוסם** (Drawing Recovery אחרי כיבוי כפוי),
   לא AutoCAD מת.
