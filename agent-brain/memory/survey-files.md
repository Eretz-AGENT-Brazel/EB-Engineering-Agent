...
⭐ **אושר על מודל שני באותו יום (קרלסברג K10):** ‏`INSUNITS=0` שוב, ו-**המדידה והסולידים שנגזרו
ממנה מוכנסים כזוג נעול** — ‏`MEDIDA` ו-`WALLS SOLID` עם **אותה נקודת הכנסה, אותו סקייל 1.000 ואותו
סיבוב** ⇒ הם לא יכולים להיפרד. ‏**הסיבוב הוא פר-פרויקט**: טארה ‎+62.0993°, קרלסברג ‎−6.178°.
⭐ ו**מחפשים את המספר שהלקוח מסר בתוך המודל**: "מסוע 550" נקרא בחזרה כגוף ב-`z 3550→4100` בדיוק.
🛑 ובדיקת התנגשות **לפי מעטפת שכבה אינה בדיקת התנגשות** — היא סינון שמפיק רשימת מועמדים; ההתנגשות
היא בין **שני גופים**, ובודקים חלק-חלק.
