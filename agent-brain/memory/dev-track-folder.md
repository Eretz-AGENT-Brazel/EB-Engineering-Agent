...
- ⚡⚡ **19/08/2026 — אין ידע מחוץ לתיקיית הפרויקט** (הוראת אמיר: *"כל הידע שלנו
  מרוכז רק שם"*). ששת המודלים המקוריים של החברה עברו מתיקייה על שולחן העבודה
  ל-**`DEV/projects/sources/`** (‏7 מ-7 `sha256` זהים), ו-`night.py`'s `SOURCE_DIR` הפך ליחסי ל-`__file__`
  — זה היה הנתיב האחרון הצרוב לשולחן העבודה. ⚠️ **הסקיל והזיכרון חייבים להישאר
  פיזית ב-`C:\Users\User\.claude`** — זה השקע ש-Claude Code טוען ממנו; התוכן ממוראי
  ב-`agent-brain/` ובדיקה 7 בשומר מפילה קומיט כשהשניים נבדלים. **לא להעביר אותם.**
