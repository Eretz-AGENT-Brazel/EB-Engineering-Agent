...
> 
> ⏳⏳ **AND `xclone` IS INSTANT ON PARTS AND PATHOLOGICALLY SLOW ON ANNOTATION.** Measured same op, same drawing: 678 profiles 5 s · 329 ProConcrete panels/slabs 2 s · 29 concrete shapes, 18 assemblies, 6 work frames and the grid **0.4-0.5 s per call** — and **200 dimensions: 2 s, then 87 s, then ~40 MINUTES**; 133 lines the same. 🛑 **I first wrote that up as a hang and it was wrong** — it shows the documented spin signature (`Responding=False`, one core, flat memory) and it **returns**. A part carries a section reference; a dimension carries dimension styles, text styles, blocks and **associativity to the geometry it measures**, so the clone drags a graph that is reconciled against everything already cloned. ⇒ **clone parts freely; treat annotation as a long job** — do not chunk it small, each call re-pays the reconciliation. ⛔ And while it runs the **whole machine is blocked**: the busy instance keeps the COM registration and the file lock, so a second AutoCAD started afterwards is unreachable (verified). **Save before a long batch.**
>
